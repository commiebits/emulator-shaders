...
<!--
	...
	Modified Joshua Street's SABR v3.0 shader with Hyllian's xBR-lv2-lq
	interpolation rules.
...
vec4 lum_wd(vec4 a, vec4 b, vec4 c, vec4 d, vec4 e, vec4 f, vec4 g, vec4 h) {
	return 0.2 * lum_df(c, d) + 0.2 * lum_df(e, f) + 0.6 * lum_df(g, h);
}
...
void main()
{
	...
	*/
	...
	vec4 e45   = lum_wd(p11, p7, p16, p12, p12, p8, p17, p13);
	vec4 econt = lum_wd(p6, p12, p7, p13, p11, p17, p12, p18);
